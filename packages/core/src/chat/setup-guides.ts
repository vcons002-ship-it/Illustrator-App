...
export const SETUP_GUIDES: SetupGuide[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "ON THE DESKTOP APP (the sign-in can't finish from a phone or the web — swapping Schwab's code for a token is a cross-origin request only the desktop can make): open the 📈 Markets panel and click Connect Schwab. It opens Schwab's consent page for YOUR app; sign in and approve, then paste the redirected https://127.0.0.1/?code=… URL back when asked — the page will show a connection error, which is expected, nothing is listening there. The app exchanges it for a token (refreshed automatically). Once connected on the desktop, a linked phone can use quotes and positions normally.",
