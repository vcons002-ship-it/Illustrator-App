...
describe("runBuddyTurn — say tool drives a message-per-step queue in ONE turn", () => {
  // A stateful working-checklist (mirrors the host's set_plan/complete_step) so the loop can run
  // exactly as the real buddy does: lay out the steps, then say + tick each one, all in-worker.
  function planDeps() {
    let plan: { goal?: string; steps: { text: string; status: "pending" | "done"; note?: string }[] } | undefined;
    return {
      ...baseDeps,
      setPlan: (goal: string | undefined, steps: string[]) => {
        plan = { ...(goal ? { goal } : {}), steps: steps.map((text) => ({ text, status: "pending" as const })) };
        return plan;
      },
  ...
  it("count to 3, one number per message: 3 say results delivered, no suspension, single turn", async () => {
    const llm = scriptedLlm([
      '{"tool":"set_plan","goal":"count to 3","steps":["say 1","say 2","say 3"]}',
      '{"tool":"say","text":"1"}\n{"tool":"complete_step","note":"sent 1"}',
      '{"tool":"say","text":"2"}\n{"tool":"complete_step","note":"sent 2"}',
    ...
    const outcome = await runBuddyTurn({
      llm,
      system: "sys",
      history: [{ role: "user", content: "count to 3, one number per message" }],
      deps: planDeps(),
      onEvent: (e: BuddyTurnEvent) => {
        if (e.kind === "toolResult" && e.result.said) said.push(e.result.said);
      },
    });
    // Every number was delivered as its own message, in order — and the whole thing ran as one
    // uninterrupted in-worker turn (never suspended waiting for the reader to say "continue").
    expect(said).toEqual(["1", "2", "3"]);
    expect(outcome.pendingTool).toBeUndefined();
    expect(outcome.text).toBe("Done — counted to 3. 🎉");
    // The say results are also in the turn's toolResults (3 says + 1 set_plan + 3 complete_steps).
    expect(outcome.toolResults.filter((r) => r.call.tool === "say").map((r) => r.result.said)).toEqual(["1", "2", "3"]);
  });

  it("a say needs no deps — it delivers even when the checklist isn't wired", async () => {
    const llm = scriptedLlm(['{"tool":"say","text":"hi there"}', "All set."]);
    const said: string[] = [];
    const outcome = await runBuddyTurn({
      llm,
      system: "sys",
    ...
    expect(outcome.text).toBe("All set.");
  });
});
