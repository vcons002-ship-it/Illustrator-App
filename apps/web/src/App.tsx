...
export function App() {
  ...
  // declared further down this file (assigned via effects, the same late-binding pattern as before
  // the extraction — now an explicit interface instead of closure coupling).
  const {
    plannerMirrorRef,
    applyPlannerRef,
    plannerCommandRef,
  ...
  // The whole local-engine lifecycle — managed-ComfyUI/A1111 start + probe + fallback resolution,
  // low-VRAM deferred start, model/LoRA/ffmpeg/text-model downloads, and the Connect handlers —
  // lives in useLocalEngine.ts. Called AFTER useRemoteMirror so the phone-relay refs it assigns
  // (connect / ffmpeg) come from the mirror's returned interface.
  const {
  ...
  const chatTurnsOf = (messages: StoredChatMessage[]): ChatTurn[] =>
    messages.flatMap((m): ChatTurn[] => {
      // WHEN each message was sent, in front of it. A conversation handed to a model has no clock in
      // it — yesterday, this morning and three weeks ago all look like the line above — which is fine
      // for one sitting and wrong for a chat kept for months, resumed from a phone and woken by
      // scheduled runs. Stamped on the READER's turns only: the model never writes those, so it can't
      // learn to imitate the format and start emitting timestamps of its own. Its own replies sit
      // directly under the message they answer.
      const stamp = (t: ChatTurn): ChatTurn =>
        t.role === "user" ? { ...t, content: stampTurnContent(t.content, m.at) } : t;
  ...
  const dispatchBuddyTurn = async (
    ...
    * record of what happened — so it keeps riding the transcript. */
    ...
    if (res.pendingTool) {
      ...
      // POLICY lives in core (routePendingTool, unit-tested); this switch only maps a route to its
      // executor. The per-tool `if` inside each case is TypeScript narrowing, not a second gate.
      const call = res.pendingTool;
      const route = routePendingTool(call.tool, {
        fileAccessGranted: fileAccessGranted.current,
        screenCaptureGranted: screenCaptureGranted.current,
        autonomousFileSearch: !!settings.autonomousFileSearch,
      ...
      switch (route) {
        case "host":
          ...
          break;
        case "image":
          if (call.tool === "generate_image") void approveGenerateImage(call);
          break;
        case "video":
          if (call.tool === "generate_video") void approveGenerateVideo(call);
          break;
        case "long-video":
          if (call.tool === "generate_long_video") void approveGenerateLongVideo(call);
          break;
        case "plan":
          if (call.tool === "plan_task") void runPlanTask(call.request);
          break;
        case "order-review":
          if (call.tool === "prep_order") openOrderReview(call);
          break;
        case "tv-chart":
          if (call.tool === "tv_chart") void runTvChart(call, [{ role: "user", content: userText }, ...res.transcript]);
          break;
        case "delegate":
